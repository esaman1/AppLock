...
import com.ezteam.baseproject.view.smoothBottombar.*
...
class SmoothBottomBar @JvmOverloads constructor(
    ...
    private var _displayAllText: Boolean = false
    ...
    private fun obtainStyledAttributes(attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        try {
            ...
            _displayAllText = typedArray.getBoolean(
                R.styleable.SmoothBottomBar_displayAllText,
                false
            )
...
//             Add ellipsis character to item text if it is shorted
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // Draw indicator
        if (_displayAllText) {
            rect.left = items[itemActiveIndex].icon.bounds.left.toFloat() - resources.getDimension(R.dimen._5sdp)
            rect.right =
                rect.left + items[itemActiveIndex].icon.bounds.width() + paintText.measureText(items[itemActiveIndex].title) + _barSideMargins + resources.getDimension(R.dimen._10sdp)
        } else {
...
//        rect.right = indicatorLocation + itemWidth + itemTextSize
